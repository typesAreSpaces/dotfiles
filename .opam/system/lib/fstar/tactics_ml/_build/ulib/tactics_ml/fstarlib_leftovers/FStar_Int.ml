...
let (incr_mod : Prims.pos -> unit int_t -> unit int_t) =
  fun n  ->
    fun a  -> (a + Prims.int_one) mod (Prims.pow2 (n - Prims.int_one))
...
let (add_underspec : Prims.pos -> unit int_t -> unit int_t -> unit int_t) =
  fun n  ->
    fun a  -> fun b  -> if fits (a + b) n then a + b else Prims.int_zero
...
let (div_underspec : Prims.pos -> unit int_t -> unit int_t -> unit int_t) =
  fun n  ->
    fun a  ->
      fun b  ->
        if fits (op_Slash a b) n then op_Slash a b else Prims.int_zero
