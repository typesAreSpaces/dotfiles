open Prims
type u8 = FStar_UInt8.t
type u16 = FStar_UInt16.t
type u32 = FStar_UInt32.t
type u64 = FStar_UInt64.t
type u128 = FStar_UInt128.t
let (htole16 : FStar_UInt16.t -> FStar_UInt16.t) =
  fun uu____9  -> failwith "Not yet implemented:htole16" 
let (le16toh : FStar_UInt16.t -> FStar_UInt16.t) =
  fun uu____19  -> failwith "Not yet implemented:le16toh" 
let (htole32 : FStar_UInt32.t -> FStar_UInt32.t) =
  fun uu____29  -> failwith "Not yet implemented:htole32" 
let (le32toh : FStar_UInt32.t -> FStar_UInt32.t) =
  fun uu____39  -> failwith "Not yet implemented:le32toh" 
let (htole64 : FStar_UInt64.t -> FStar_UInt64.t) =
  fun uu____49  -> failwith "Not yet implemented:htole64" 
let (le64toh : FStar_UInt64.t -> FStar_UInt64.t) =
  fun uu____59  -> failwith "Not yet implemented:le64toh" 
let (htobe16 : FStar_UInt16.t -> FStar_UInt16.t) =
  fun uu____69  -> failwith "Not yet implemented:htobe16" 
let (be16toh : FStar_UInt16.t -> FStar_UInt16.t) =
  fun uu____79  -> failwith "Not yet implemented:be16toh" 
let (htobe32 : FStar_UInt32.t -> FStar_UInt32.t) =
  fun uu____89  -> failwith "Not yet implemented:htobe32" 
let (be32toh : FStar_UInt32.t -> FStar_UInt32.t) =
  fun uu____99  -> failwith "Not yet implemented:be32toh" 
let (htobe64 : FStar_UInt64.t -> FStar_UInt64.t) =
  fun uu____109  -> failwith "Not yet implemented:htobe64" 
let (be64toh : FStar_UInt64.t -> FStar_UInt64.t) =
  fun uu____119  -> failwith "Not yet implemented:be64toh" 
type ('Aa,'Arrel,'Arel,'Ab,'Ai,'Aj,'Apredicate,'Ah) store_pre = unit
type ('Aa,'Arrel,'Arel,'Ab,'Ai,'Aj,'Apredicate,'Ah0,'Auu____309,'Ah1) store_post =
  unit
let store16_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt16.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store16_le_i" 
let load16_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt16.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load16_le_i" 
let store16_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt16.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store16_be_i" 
let load16_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt16.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load16_be_i" 
let store32_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store32_le_i" 
let load32_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load32_le_i" 
let store32_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store32_be_i" 
let load32_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load32_be_i" 
let store64_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store64_le_i" 
let load64_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load64_le_i" 
let store64_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t -> unit
  = fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store64_be_i" 
let load64_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load64_be_i" 
let store128_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt128.t -> unit
  =
  fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store128_le_i" 
let load128_le_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt128.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load128_le_i" 
let store128_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt128.t -> unit
  =
  fun b  -> fun i  -> fun z  -> failwith "Not yet implemented:store128_be_i" 
let load128_be_i :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt128.t
  = fun b  -> fun i  -> failwith "Not yet implemented:load128_be_i" 
let (store16_le :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt16.t -> unit) =
  fun b  ->
    fun z  -> store16_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load16_le : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt16.t) =
  fun b  -> load16_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store16_be :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt16.t -> unit) =
  fun b  ->
    fun z  -> store16_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load16_be : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt16.t) =
  fun b  -> load16_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store32_le :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt32.t -> unit) =
  fun b  ->
    fun z  -> store32_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load32_le : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt32.t) =
  fun b  -> load32_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store32_be :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt32.t -> unit) =
  fun b  ->
    fun z  -> store32_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load32_be : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt32.t) =
  fun b  -> load32_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store64_le :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt64.t -> unit) =
  fun b  ->
    fun z  -> store64_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load64_le : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt64.t) =
  fun b  -> load64_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (load64_be : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt64.t) =
  fun b  -> load64_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store64_be :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt64.t -> unit) =
  fun b  ->
    fun z  -> store64_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load128_le : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt128.t) =
  fun b  -> load128_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store128_le :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt128.t -> unit) =
  fun b  ->
    fun z  -> store128_le_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let (load128_be : FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt128.t) =
  fun b  -> load128_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) 
let (store128_be :
  FStar_UInt8.t LowStar_Buffer.buffer -> FStar_UInt128.t -> unit) =
  fun b  ->
    fun z  -> store128_be_i b (FStar_UInt32.uint_to_t Prims.int_zero) z
  
let index_32_be :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t
  =
  fun b  ->
    fun i  ->
      load32_be_i b
        (FStar_UInt32.mul (FStar_UInt32.uint_to_t (Prims.of_int (4))) i)
  
let index_32_le :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t
  =
  fun b  ->
    fun i  ->
      load32_le_i b
        (FStar_UInt32.mul (FStar_UInt32.uint_to_t (Prims.of_int (4))) i)
  
let index_64_be :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t
  =
  fun b  ->
    fun i  ->
      load64_be_i b
        (FStar_UInt32.mul (FStar_UInt32.uint_to_t (Prims.of_int (8))) i)
  
let index_64_le :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt64.t
  =
  fun b  ->
    fun i  ->
      load64_le_i b
        (FStar_UInt32.mul (FStar_UInt32.uint_to_t (Prims.of_int (8))) i)
  

let upd_32_be :
  'Arrel 'Arel .
    (FStar_UInt8.t,'Arrel,'Arel) LowStar_Monotonic_Buffer.mbuffer ->
      FStar_UInt32.t -> FStar_UInt32.t -> unit
  =
  fun b  ->
    fun i  ->
      fun v1  ->
        let h0 = FStar_HyperStack_ST.get ()  in
        store32_be_i b
          (FStar_UInt32.mul (FStar_UInt32.uint_to_t (Prims.of_int (4))) i) v1;
        (let h1 = FStar_HyperStack_ST.get ()  in ())
  